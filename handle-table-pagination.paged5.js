/* handle-table-pagination.paged3.js - stable: repeat table headers + continued label + orphan note fix */
class RepeatTableHeadersHandler extends Paged.Handler {
  ...
  layout(rendered, layout) {
    // 1) Fix orphan mm-table-note at top of a page (note moved alone to next page)
    ...
    this.splitTablesRefs.forEach(ref => {
      ...
      // Important: only act on continued fragments (those usually miss THEAD after splitting)
  ...
  // If the first meaningful element on the page is a mm-table-note, give it a continued label + correct padding
  fixOrphanNoteAtTop(rendered) {
    const firstBlock = this.getFirstMeaningfulElement(rendered)
    if (!firstBlock) return

    if (!(firstBlock.classList && firstBlock.classList.contains("mm-table-note"))) return
    ...
    const alreadyHasSpacer = prev && prev.classList && prev.classList.contains("mm-table-continued-spacer")
    if (!alreadyHasSpacer) {
      const title = this.findPrevH2Text(note)
        ? `${this.findPrevH2Text(note)} - forts. från föregående sida`
      ...
      // Inline style to prevent the "too high up" problem even if CSS selector misses due to wrappers
      spacer.style.marginTop = "40px"
      spacer.style.paddingLeft = "var(--facts-pad-x)"
      spacer.style.paddingRight = "var(--facts-pad-x)"

      note.parentNode.insertBefore(spacer, note)
    }

    // Inline padding for the note as well, same reason as above
    note.style.paddingLeft = "var(--facts-pad-x)"
    note.style.paddingRight = "var(--facts-pad-x)"
  }
