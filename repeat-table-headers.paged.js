...
\f0\fs24 \cf0 /* \
  repeat-table-headers.paged.js\
  ---------------------------------------\
  Workaround for repeating <thead> on multipage tables\
  when using Paged.js (e.g. in pdfMonkey).\
\
  Usage:\
  1. Load paged.polyfill.js\
  2. Load this file AFTER paged.polyfill.js\
*/\
\
...
\
    if (thead) \{\
      const lastTheadNode = thead.hasChildNodes()\
        ? thead.lastChild\
        : thead\
\
...
\
  hideEmptyTables(pageElement, breakTokenNode) \{\
    this.splitTablesRefs.forEach(ref => \{\
      const table = pageElement.querySelector(\
        "[data-ref='" + ref + "']"\
      )\
\
      if (!table) return\
...
\
      if (\
        !firstRow ||\
        this.refEquals(firstRow.firstElementChild, breakTokenNode)\
      ) \{\
